define(['angular'], function(angular) {
	...
	contactList.run(function($http){
        $http.defaults.headers.common["Content-Type"] = "application/json";
    });
	
	contactList.controller('contactListCtrl', function($scope, $rootScope, $http, $timeout, $location, $filter,$mdDialog) {
		...
		console.log($scope.pageNum+$scope.pageSize);
		$scope.findList = function() {
			$http({
					method: 'POST',
					url: adminUrl + "contact/getList",
					data: {
		...
		$scope.goEdit = function(id) {
			$location.url('/contact/edit?id=' + id);
		}
